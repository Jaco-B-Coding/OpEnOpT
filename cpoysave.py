...
class Optimization_model():
    ...
    def __init__(self, simulation_steps, time_step, simulation, opt_pv_size, opt_batt_size):
        ...
        self.deviation_threshold = 0.001
    ...
    def update_model_data(self, eco_pv, eco_bat):
        ...
        if self.sim.pv is not None:            
            ...
            for i in range(self.num_pv_sources):
                ...
                self.pv_used_power.append(self.sim.pv_power[i])
    ...
    def optimize_model(self):
        ...
        def demand_init_rule(m,t):
            return self.demand[t-1]
        self.model.demand = pyo.Param(self.model.simulation_steps, initialize = demand_init_rule)
        
        def buyprice_init_rule(m,t):
            return self.buyprice[t-1]
        self.model.buyprice = pyo.Param(self.model.simulation_steps, initialize = buyprice_init_rule)
        
        def sellprice_init_rule(m,t):
            return self.sellprice[t-1]
        self.model.sellprice = pyo.Param(self.model.simulation_steps, initialize = sellprice_init_rule)
        ...
        def buy_rule(m,t):
            if not self.bought_power_list:
                ...
                return self.bought_power_list[t-1]
        ...
        def sell_rule(m,t):
            if not self.sold_power_list:
                ...
                #init with old iteration values
                return self.sold_power_list[t-1]
        ...
        def shortage_power_rule(m,t):
            if not self.power_shortage_list:
                ...
                #init with old iteration values
                return self.power_shortage_list[t-1]
        ...
        def pv_comp_rule (model_block, model_set):
            ...
            model_block.pv_total_LCOE_factor = pyo.Param(initialize = self.pv_total_LCOE_factor[model_set-1])
            model_block.pv_cost = pyo.Var( initialize = self.LCOE_pv[model_set-1], within = pyo.NonNegativeReals)
            
            model_block.pv_array_kWp = pyo.Param( initialize = self.pv_array_kWp[model_set-1])
            model_block.max_pv_kWp = pyo.Param( initialize = self.max_pv_peak_power[model_set-1])
            
            def pv_power_rule(_model_block, t):
                return self.pv_max_power[model_set-1][t-1]                       #for different power data for different sources: return pv_max_power[i][t-1] for i in _self.model_block.pv_sources_set
            model_block.pv_max_power = pyo.Param(self.model.simulation_steps, initialize = pv_power_rule)
            
            def pv_used_power_rule(m, t):
                return self.pv_used_power[model_set-1][t-1]
            model_block.pv_used_power = pyo.Param(self.model.simulation_steps, initialize = pv_used_power_rule)
            ...
            if self.opt_pv_size == True:
                model_block.pv_peak_mod = pyo.Var(initialize = 1, bounds = (0,10))
            else:
                model_block.pv_peak_mod = pyo.Param(initialize = 1)
        ...
        def battery_comp_rule (model_block, model_set):
            ...
            model_block.bat_total_LCOE_factor = pyo.Param(initialize = self.bat_total_LCOE_factor)
            model_block.battery_charge_cost = pyo.Var(initialize = self.LCOE_battery_charge, within = pyo.NonNegativeReals)
            model_block.battery_discharge_cost = pyo.Param(initialize = self.LCOE_battery_discharge)
            
            model_block.bat_array_kWp = pyo.Param( initialize = self.battery_array_nominal_capacity)
            model_block.max_battery_capacity = pyo.Param( initialize = self.max_battery_capacity)
            
            if self.opt_batt_size == True:
                model_block.battery_peak_mod = pyo.Var( initialize = 1, bounds = (0.5,3))
            else:
                model_block.battery_peak_mod = pyo.Param( initialize = 1)
            ...
            def battery_discharge_rule(m,t):
                if not self.battery_discharge_list:
                    ...
                    #init with old iteration values
                    return self.battery_discharge_list[t-1]
            ...
            def battery_charge_rule(m,t):
                if not self.battery_charge_list:
                    ...
                    #init with old iteration values
                    return self.battery_charge_list[t-1]
            ...
            def battery_charged_power_rule(m,t):
                return self.battery_charged_power[t-1]
            ...
            def battery_array_capacity_rule(_model_block, t):
                return self.battery_capacity_current_wh[t-1]
        ...
        # opt.options['max_iter']= 10000 #number of iterations you wish
        # opt.options['linear_solver'] = 'ma86'
        # results = opt.solve(self.model, tee=False)   #set tee to True if solver output needs to be printed
        
        # self.model.pprint()        
        
        pyo.SolverFactory('mpec_minlp').solve(self.model)
        ...
        # print(pyo.value(self.model.pv_comp_block[1].pv_peak_mod))
                
        b=1
        
        print('pvLCOE:',pyo.value(self.model.pv_comp_block[b].pv_cost)*1000)
        print('battery LCOE', pyo.value(self.model.battery_comp_block[b].battery_charge_cost)*1000)
        print('buy cost:', pyo.value(self.model.buyprice[1])*1000)
        print('shortage LCOE:',pyo.value(self.model.shortage_costs)*1000)
        
        self.pv_peak_mod_list = list()
        for i in range(len(self.sim.pv)):
            self.pv_peak_mod_list.append(pyo.value(self.model.pv_comp_block[i+1].pv_peak_mod))
        print('pv_peak_mod:', self.pv_peak_mod_list)
        self.batt_peak_mod = pyo.value(self.model.battery_comp_block[b].battery_peak_mod) 
        print('battery_peak_mod:',self.batt_peak_mod)
        
        
        self.pv_max_possible_power = list()
        for i in range(len(self.sim.pv)):
            mult_workaround = pyo.value(self.model.pv_comp_block[i+1].pv_peak_mod)
        ...
        for t in self.model.simulation_steps:
            for j in range(len(self.sim.pv)):  
                ...
                energy_creation += pyo.value(self.model.pv_comp_block[j].pv_max_power[t]*self.model.pv_comp_block[j].pv_module_power[t]*self.model.pv_comp_block[j].pv_charger_efficiency[t]\
                                             *self.model.pv_comp_block[j].pv_peak_mod)
        ...
        # for t in self.model.simulation_steps:
        #     expr = 0
        #     expr += self.model.pv_comp_block[b].pv_max_power[t]*self.model.pv_comp_block[b].pv_module_power[t]* self.model.pv_comp_block[b].pv_charger_efficiency[t]*self.model.pv_comp_block[b].pv_peak_mod
    ...
    def get_opt_values(self):
        ...
        for t in self.model.simulation_steps:
            #pv
            for i in range(len(self.sim.pv)):
                peak_mod_workaround = [pyo.value(self.model.pv_comp_block[i+1].pv_peak_mod)]
                expr=peak_mod_workaround[0]*pyo.value(self.model.pv_comp_block[i+1].pv_max_power[t]*self.model.pv_comp_block[i+1].pv_module_power[t])
                self.pv_flow[i].append(expr)
                
            #unused pv power     --> add self.model.pv_comp_block[i+1].pv_peak_mod*
            expr = sum(pyo.value((self.model.pv_comp_block[j+1].pv_peak_mod*self.model.pv_comp_block[j+1].pv_max_power[t] \
                             -self.model.pv_comp_block[j+1].pv_max_power[t]*self.model.pv_comp_block[j+1].pv_module_power[t]\
                             *self.model.pv_comp_block[j+1].pv_peak_mod)) for j in range(len(self.sim.pv)))
            ...
            self.battery_charge_list.append(pyo.value(self.model.battery_comp_block[b].battery_current_charge_power[t]))
            self.battery_discharge_list.append(pyo.value(self.model.battery_comp_block[b].battery_current_discharge_power[t]))
            
            #power junction
            expr = sum(pyo.value(self.model.pv_comp_block[j+1].pv_max_power[t]*self.model.pv_comp_block[j+1].pv_module_power[t]\
                                 *self.model.pv_comp_block[j+1].pv_charger_efficiency[t])for j in range(len(self.sim.pv)))\
            ...
            expr = pyo.value(self.model.grid_current_bought_power[t])
            self.bought_power_list.append(expr)
            expr = pyo.value(self.model.grid_current_sold_power[t])
            self.sold_power_list.append(expr)
            
            #power shortage
            expr = pyo.value(self.model.shortage_power[t])
