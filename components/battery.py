...
class Battery(Serializable, Simulatable):
    ...
    def __init__(self,
        ...
        # Initialize starting paremeters
        self.state_of_charge_init = 0.9
        self.state_of_charge = self.state_of_charge_init
        #minimal efficiency
        self.minimal_efficiency = 0.8
        self.const_efficiencies = False      #toggle whether efficiencies to be kept constant
        self.const_ch_eff = 0.9
        self.const_dch_eff = 0.9
        ...
        ##pyomo model optimization
        #aimed for level of utilization
        self.min_used_capacity = 0.9
...
# =============================================================================
    ...
    def calculate(self):
        ...
        #get available input link power (from battery management)
        self.power = self.input_link.power
        self.input_link_power = self.input_link.power
        ...
        if self.input_link_power < 0:
            ...
            if (self.state_of_charge < self.charge_discharge_boundary):
                ...
                self.power_battery = self.power + ((abs(self.state_of_charge - self.charge_discharge_boundary)
                                        - self.power_self_discharge_rate) * self.capacity_current_wh / (self.timestep/3600))
                self.power_battery = round(self.power_battery,4)
                ...
                self.input_link.power = self.power_battery * self.discharging_efficiency 
        ...
        # Charge case
            ...
            if (self.state_of_charge > self.charge_discharge_boundary):
                ...
                # Set new input_link (BMS) power
                self.input_link.power = self.power_battery / self.charging_efficiency
                # Calculate new input_link (BMS) efficiency at new power level
    ...
    def battery_power(self):
        ...
        if self.const_efficiencies:
            if self.power > 0.: #charge
                self.charging_efficiency = self.const_ch_eff
                self.power_battery = self.power * self.charging_efficiency
                
                self.discharging_efficiency = self.const_dch_eff
                ...
                self.discharging_efficiency = self.const_dch_eff
                self.power_battery = self.power / self.discharging_efficiency
            ...
            if self.power > 0.: #charge
                self.charging_efficiency = self.charge_power_efficiency_a * (self.power/self.capacity_nominal_wh) + self.charge_power_efficiency_b
                self.power_battery = self.power * self.charging_efficiency
                
                self.discharging_efficiency = self.minimal_efficiency
        
            elif self.power == 0.: #idle
                self.charging_efficiency = self.minimal_efficiency
                self.discharging_efficiency = self.minimal_efficiency
                self.power_battery = self.power * self.charging_efficiency
        
            elif self.power < 0.: #discharge
                self.discharging_efficiency = self.discharge_power_efficiency_a*(abs(self.power)/self.capacity_nominal_wh) + self.discharge_power_efficiency_b
                self.power_battery = self.power / self.discharging_efficiency
