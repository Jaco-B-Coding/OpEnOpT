...
class Optimization_model():
    ...
    def __init__(self, simulation_steps, time_step, simulation, opt_pv_size, opt_batt_size):
        ...
        self.low_dev_in_row = 5     #to mark how many deviations in a row need to be lower than threshhold to have an answer
        ...
        self.solver_type = 'ma57'
        
        #choose efficiency formulation method
        self.poly_fit_eff = False         #fit efficiencies through polynomial fit
        self.exact_ch_eff = False          #select whether battery ch/dch efficiency to be implemented as linear fit in Pyomo
        
        #only constant eff  --> disable other methods
        self.const_eff = False
        if self.const_eff:
            #choose other eff modeling method if not constant
            #fit efficiencies through polynomial fit
            self.poly_fit_eff = False
            #select whether battery ch/dch efficiency to be implemented as linear fit in Pyomo
            self.exact_ch_eff = False
    ...
    def init_model(self):
        ...
        def pv_comp_rule (model_block, model_set):
            ...
            elif self.const_eff == True:
                model_block.pv_charger_efficiency = pyo.Param(self.model.simulation_steps, initialize = 1)
            else:
        ...
        def battery_comp_rule (model_block, model_set):
            ...
            if self.poly_fit_eff:
                ...
                model_block.battery_charger_efficiency = pyo.Param(self.model.simulation_steps, initialize = 1)   
            else:
            ...
            if self.poly_fit_eff:
                ...
                model_block.battery_discharger_efficiency = pyo.Param(self.model.simulation_steps, initialize = 1)   
            else:
            ...
            if self.exact_ch_eff:
                ...
                model_block.battery_charging_efficiency = pyo.Param(self.model.simulation_steps, initialize = 0.8)
            else:
            ...
            if self.exact_ch_eff:
                ...
                model_block.battery_discharging_efficiency = pyo.Param(self.model.simulation_steps, initialize = 0.8)
            else:
        ...
        def pv_LCOE_rule(m, b):
            ...
            expr =  ((m.pv_comp_block[b].pv_total_LCOE_factor) * m.pv_comp_block[b].pv_array_kWp\
                ...
                *(m.battery_comp_block[b].bat_total_LCOE_factor )) \
